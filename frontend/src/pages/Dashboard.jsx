                    ...
                    dataToShow.map((item) => (
                        <div key={item.id} className="card" style={{ width: '100%', flexDirection: 'column' }}>
                            <div style={{ position: 'relative', width: '100%', height: '160px', backgroundColor: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                                ...
                                <h3 style={{ margin: 0, fontSize: '1.5rem', color: 'var(--text-main)', zIndex: 2, padding: '0 1rem', textAlign: 'center', fontWeight: '800' }}>{item.destination}</h3>
                            </div>
                            <div className="card-content" style={{ display: 'flex', gap: '1.5rem', padding: '1.5rem', flexDirection: 'row' }}>
                                <div style={{ flexShrink: 0 }}>
                                    <img src={getFullImageUrl(item.travelerImage) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${item.travelerName || 'Traveler'}`} alt={item.travelerName || 'Traveler'} style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--primary-color)' }} />
                                </div>
                                <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                                    <h4 style={{ margin: '0 0 0.2rem 0', fontSize: '1.2rem', color: 'var(--primary-hover)' }}>{item.travelerName || 'Traveler'}</h4>
                                    <p style={{ margin: '0 0 0.8rem 0', fontWeight: 'bold', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>🗓️ {item.tripDates}</p>
                                    <p style={{ margin: '0 0 1rem 0', fontSize: '0.95rem', color: 'var(--text-main)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', fontStyle: 'italic', lineHeight: '1.5' }}>"{item.description}"</p>
                                    <button
                                        className="cta-button outline"
